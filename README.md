# Banco de Dados Clientes — Sistema de Gestão de Clientes (mínimo)
...
4. Abra o frontend no navegador: `http://localhost:3000/` ou abra `frontend/index.html` diretamente.

Instalação simplificada

- Passos rápidos após clonar o repositório:

```powershell
# no Windows (PowerShell)
npm install    # instala dependências via workspaces (backend)
...
# Banco de Dados Clientes — Sistema de Gestão de Clientes

Visão geral

Este repositório contém um sistema simples de gerenciamento de clientes com autenticação (backend em Node/Express e frontend estático). O objetivo é ser fácil de executar localmente para desenvolvimento e demonstração.

Principais características
- Autenticação com JWT (`/api/register`, `/api/login`).
- CRUD de clientes (nome, email, telefone, foto).
- Modo offline: fallback para `localStorage` quando o backend estiver indisponível.
- Medidas básicas de segurança (headers, rate limiting, validação, uploads limitados).

Conteúdo do repositório
- `backend/` — servidor Express, persistência em `backend/data.json`, uploads em `backend/uploads`.
- `frontend/` — arquivos estáticos (HTML/CSS/JS) consumindo a API.
- `docs/` — documentação adicional (instalação, guia do usuário, segurança).
